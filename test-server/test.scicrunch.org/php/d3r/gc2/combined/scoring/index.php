<?php
    ...
    */
    ...
    if ((isset($_GET['component'])) && (in_array($_GET['component'], array("417", "443")))) {
        ...
        switch ($component) {
            case "417":
                $title .= "Scoring Method (Stage 1)";
                $stage = 1;
                $include_method = "scoring_stage1";
                break;

            case "443":
                $title .= "Scoring Method (Stage 2)";
                $stage = 2;
                $include_method = "scoring_stage2";
                break;
        }
        $title .= " - Kendall's Tau";
    ...
    $json_filter = "/php/d3r/gc2/combined/scoring/json_filter.php?" . implode('&', $parameters);
...
function drawChart(method, block_id){
    ...
    var tip = d3.tip()
      ...
      .offset([-10, 0])
      .html(function(d) {
        return "<strong>" + d.label + "<br />Kendall's Tau:</strong> " + d.value.toPrecision(2) + " +/-" + d.moe.toPrecision(2) + " (n=" + d.n + ")";
      })
    ...
    <h2>Affinity Ranking (Stage <?php echo $stage; ?>) - Kendall's Tau</h2>
    <div id="ligand_block"></div>

    <script type="text/javascript">
        drawChart("ligand", "#ligand_block");
    </script>
