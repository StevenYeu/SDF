<?php
$user = new User();
$user->getByID($component_data->uid);

$tags = $component_data->getTags();
$holder = new Tag();
$popular = $holder->getPopularTags($thisComp->component, $community->id, 0, 15);

$tagURL = "/" . $community->portalName . "/about/" . $thisComp->text2 . "?tag=";

if($component_data->component()->icon1 == "event1") {
    $content = $component_data->description;
} else {
        ...
        <div class="col-md-9 md-margin-bottom-60 <?php if($vars['editmode']) echo 'editmode' ?>">
            <!--Blog Post-->
                <div class="blog margin-bottom-40">
                    <h1><?php echo Component::macroExpand($component_data->title, Array("community" => $community)) ?></h1>

                    <div class="blog-post-tags">
                        <ul class="list-unstyled list-inline blog-info">
                            <li><i class="fa fa-calendar"></i> <?php echo date('h:ia F j, Y', $component_data->time) ?></li>
                        ...
                        <ul class="list-unstyled list-inline blog-tags" style="margin-top: 10px">
                            <li>
                                ...
                                <?php foreach ($tags as $tag) {
                                    echo '<a style="margin:0 3px" href="' . $tagURL . $tag->tag . '">' . $tag->tag . '</a>';
                                }?>
                            </li>
                        </ul>
                    </div>
                    <?php echo Component::macroExpand($content, Array("community" => $community)) ?>
                </div>
            <!--End Blog Post-->
            ...
            <?php if ($vars['editmode']) {
                ...
                echo '<button class="btn-u btn-u-default edit-body-btn" componentType="data" componentID="'.$component_data->id.'"><i class="fa fa-cogs"></i><span class="button-text"> Edit</span></button><a href="javascript:void(0)" componentID="'.$component_data->id.'" community="'.$community->id.'" class="btn-u btn-u-red article-delete-btn"><i class="fa fa-times"></i><span class="button-text"> Delete</span></a></div>';
                echo '</div>';
                if (count($tags) > 0) {
                    foreach ($tags as $tag) {
                        $tagText[] = $tag->tag;
                ...
                <div class="custom-form back-hide no-padding">
                    ...
                    <form method="post" action="/forms/component-forms/component-update.php?id=<?php echo $component_data->id ?>"
                          id="header-component-form" class="sky-form" enctype="multipart/form-data">
                        <div class="row margin-bottom-10">
                            <fieldset>
                                <?php

                                echo $component_data->getContainerDataForm($thisComp->icon1, $tt);
                                ?>
                            </fieldset>
                        </div>
                ...
                <div class="article-delete back-hide">
                    ...
                    <a href="/forms/component-forms/body-data-delete.php?cid=<?php echo $community->id ?>&component=<?php echo $component_data->id ?>" class="btn-u btn-u-default" style="">Yes</a>
        ...
        <div class="col-md-3 magazine-page">
            ...
            <div class="posts margin-bottom-40">
                <div class="headline headline-md">
                    <h2>Recent <?php echo $thisComp->text1 ?></h2>
                </div>
                <?php
                $datas = $component_data->getByComponentNewest($thisComp->component, $community->id, 0, 3);
                foreach ($datas as $compData) {
                    ...
                    echo '<dd>
                    <p><a href="'.$baseURL . $thisComp->text2 . '/' . $compData->id . '">' . Component::macroExpand($compData->title, Array("community" => $community)) . '</a></p>
                </dd>
            </dl>';
                }
                ?>
