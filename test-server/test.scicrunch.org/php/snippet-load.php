...
$url = Connection::environment() . '/v1/federation/data/' . $view . '.xml?count=1&exportType=all';
$xml = simplexml_load_file($url);

?>
<div class="close dark less-right" style="z-index:100">X</div>
        ...
        <form method="post"
            ...
            <fieldset>
                ...
                <section>
                    ...
                    <label class="input">
                        <i class="icon-append fa fa-question-circle"></i>
                        <input type="text" class="snippet-citation" name="citation" placeholder="Focus to view the tooltip"
        ...
        $('.test-snippet').click(function () {
            $('.testing-zone').load(
                '/php/snippet-test.php?view=<?php echo $view?>&title=' + encodeURIComponent($('.snippet-title').val())
                    + '&desc=' + encodeURIComponent($('.snippet-description').val()) + '&url=' + encodeURIComponent($('.snippet-url').val()) + '&citation=' + encodeURIComponent($('.snippet-citation').val())
            );
        });
    });
</script>
