...
class Snippet extends Connection {
    ...
    public function splitParts(){
        ...
        if ($xml) {
            ...
            if (count($splits) > 1) {
                ...
                if(count($splitter) > 1) {
                    $this->snippet["url"] = html_entity_decode(trim(str_replace("<url>", "", str_replace("</url>", "", $splitter[1]))));
                }
            } else {
    ...
    public function getSnippetByView($cid,$view){
        ...
        if(count($return) == 0) $return = $this->select('snippets',array('snippet'),'s',array($view),'where cid=0 and view=? and snippet != ""');
        if(count($return) == 0) $return = $this->select('snippets',array('snippet'),'s',array($view),'where view=? and snippet != "" order by time asc limit 1');
        if(count($return)>0){
    ...
    public function hasSnippet($cid, $view){
        $this->connect();
        $return = $this->select('snippets',array('snippet'),'is',array($cid,$view),'where cid=? and view=?');
        $this->close();
