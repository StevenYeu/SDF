...
class Collection extends Connection {
    ...
    static public function checkInAnyCollection($uuid, $user) {
        if(!$user) return false;
        $cxn = new Connection();
        $cxn->connect();
        $count = $cxn->select("collected", Array("count(*)"), "si", Array($uuid, $user->id), "where uuid=? and uid=?");
        $cxn->close();
        return $count[0]["count(*)"] > 0;
    }

    static public function checkInCollection($uuid, $user) {
        if(!$user) return false;
        return self::checkInAnyCollection($uuid, $user) || RRIDReportItem::checkInAnyReport($uuid, $user);
    }
