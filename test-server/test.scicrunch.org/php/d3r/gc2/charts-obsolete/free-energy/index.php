<?php
    ...
    */
    ...
    if ((isset($_GET['component'])) && (in_array($_GET['component'], array("417", "443")))) {
        ...
        switch ($component) {
            case "417":
                $title = "Free Energy Set $set (Stage 1)";
                break;

            case "443":
                $title = "Free Energy Set $set (Stage 2)";
                break;
        }
    } else {
    ...
    if (!(isset($_GET['q']))) {
        ...
        $which = 'RMSD';
        $which_label = 'RMSD (kcal/mol)';
    } elseif ($_GET['q'] == 'kendall') {
        $which = "Kendall's Tau";
        $which_label = "Kendall's Tau";
    }
    ...
    $json_filter = "/php/d3r/gc2/charts-obsolete/free-energy/json_filter.php?" . implode('&', $parameters);
