...
$app->match($AP."/scigraph/{path}", function(Request $request, $path) use($app) {
    require_once __DIR__ . "/scigraphservices_wrapper.php";

    $real_path = str_replace("/api/1/scigraph/", "", $request->getPathInfo());
...
$app->get($AP."/scigraph-service/annotations-entities.tsv", function(Request $request) use($app) {
    require_once __DIR__ . "/scigraph-custom-services.php";
