...
//ini_set("display_errors", 1);
    ...
    $dataa = new Challenge_Submission();

    $proto = $dataa->getProtocolsFromSubmissionID($sub['id']);
    $path = '/var/www/d3r-files/evaluation-results/gc4_protocol_files';

    $load_files = array();

    foreach ($proto as $p) {
        $folder = $p['component'] ."/";
        $load_files[] = $folder . $p['filename'];
    }

    foreach ($load_files as $file) {
        $f_split = explode("/", $file);
        $folder = $f_split[0];
        $file = $f_split[1];
