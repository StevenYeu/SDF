                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            foreach ($_SESSION['user']->collections as $id => $collection) {
                                ...
                                $cxn = new Connection();
                                $cxn->connect();
                                $last_collected = $cxn->select("collected", Array("time"), "ii", Array($collection->id, $_SESSION["user"]->id), "where collection=? and uid=? order by time desc limit 1");
                                $cxn->close();
                                if($last_collected) {
                                    $last_updated_time = $last_collected[0]["time"];
                                }
                                ...
                                if($last_updated_time) {
                                    echo '<td>' . date('h:ia F j, Y', $last_updated_time) . '</td>';
                                } else {
                                    echo '<td></td>';
                                }
                                echo '<td>';
