...
class Component extends Connection {
    ...
    public $disabled;
    public $time;
    public $include_scicrunch;

    public $visibility;
    public $rules;

    static private $comColors = [];
    public $start;
    public $end;

    public static $listable = array(
        'timeline1', 'timeline2', 'event1', 'files1', 'blog1','gallery1', 'slideshow1', 'series1', 'challengeset1', 'doi');
    ...
    public $component_ids = array(
        ...
        15 => 'Highlight Panel 1',
        16 => 'Highlight Panel 2',
        17 => 'Term Search Banner',
        21 => 'Goto Block',
        22 => 'News Thumbnails',
        23 => 'Calendar Block',
        24 => 'Search Bar',
        ...
        92 => 'Base Footer',
        100 => 'Bread Crumbs',
        41 => 'Software Search Box' // added by Steven
    );
    ...
    public $component_descs = array(
        ...
        15 => 'Highlight Panel 1.',
        16 => 'Highlight Panel 2.',
        17 => 'A term search bar above a background image you upload so direct users to the proper place in Community
               Resources.',
        ...
        37 => 'A scrolling list of sponsors (or any other list of items).',
        38 => 'News items automatically populated with updates containing a specific tag',
        39 => 'A static component for labs on the front page',
        40 => 'A static component for the resource information network',
        41 => 'A search box that search through all curated Software Tools' // added by Steven
    );
...
//        'challenge1' => 'Challenge'
    ...
    public $container_descs = array(
        ...
        'challengeset1' => 'Page for a "Challenge Set" for a Series Challenge',
        'doi' => 'Derived from a static page, this one is for DOI downloads'
        /*
    ...
    //checks to see if a container is one of the ones with listable elements
    public function isListable(){

        if(in_array($this->icon1, self::$listable)){
            return true;
        }else{
            return false;
        }
    ...
    //builds URL for passed in container, builds new one if invalid
    public function buildURL($community=NULL){

        $comm_local = $community;

        if($community==NULL || $community!=$this->cid){
            $comm_local= new Community();
            $comm_local->getByID($this->cid);
        }
        return  Community::fullURLStatic($comm_local).'/about/'.$this->text2;
    }
    ...
    public function create($vars) {
        ...
        $this->time = time();
        $this->include_scicrunch = isset($vars['include_scicrunch']) ? $vars['include_scicrunch'] : 0;
    ...
    public function updateData($vars) {
        ...
        $this->icon3 = $vars['icon3'];
        $this->disabled = $vars['disabled'];
        if(isset($vars['include_scicrunch'])) $this->include_scicrunch = $vars['include_scicrunch'];
    }
    ...
    public function createFromRow($vars) {
        ...
        $this->disabled = $vars['disabled'];
        $this->time = $vars['time'];
        $this->include_scicrunch = $vars['include_scicrunch'];
    }

    public function insertDB() {
        $this->connect();
        $this->id = $this->insert('community_components', 'iiiiisssssssssssiii', array(null, $this->uid, $this->cid, $this->component, $this->position, $this->type, $this->image, $this->text1, $this->text2, $this->text3, $this->color1, $this->color2, $this->color3, $this->icon1, $this->icon2, $this->icon3, $this->disabled, $this->time, $this->include_scicrunch));
        $this->close();
    }

    public function updateDB() {
        $this->connect();
        $this->update('community_components', 'issssssssssiii', array('component', 'image', 'text1', 'text2', 'text3', 'color1', 'color2', 'color3', 'icon1', 'icon2', 'icon3', 'disabled', 'include_scicrunch'), array($this->component, $this->image, $this->text1, $this->text2, $this->text3, $this->color1, $this->color2, $this->color3, $this->icon1, $this->icon2, $this->icon3, $this->disabled, $this->include_scicrunch, $this->id), 'where id=?');
        $this->close();
    }
    ...
    public function getDynamicStatus() {
        if ($this->component == 10 || $this->component == 13 || $this->component == 15 || $this->component == 16 ||$this->component == 22 || $this->component == 30 || $this->component == 33 || $this->component == 35 || $this->component == 37)
            return true;
        else return false;
    }
    ...
    public function getByIcon3($cid, $icon3) {
        $this->connect();
        $return = $this->select('community_components as components left join component_data as data on components.component=data.component left join extended_data as ext on data.id = ext.data', array('text3, text1, title, start, end, file, name, data.id as did, ext.id as ext_id, components.id as comp_id'), 'ii', array($icon3, $cid), 'where components.icon3=? and components.cid=? order by components.position, data.id');
        $this->close();
        ...
        if (count($return) > 0) {
            foreach ($return as $row) {
                ...
                $data->text3 = $row['text3'];
                $data->file = $row['file'];
                $data->name = $row['name'];
                $data->did = $row['did'];
                $data->ext_id = $row['ext_id'];
                $data->ext_desc = $row['ext_desc'];
    ...
    public function getByIcon1($cid, $icon1) {
        $this->connect();
        $rows = $this->select("community_components", Array("*"), "is", Array($cid, $icon1), "where cid=? and icon1=?");
        $this->close();

        $finalArray = Array();
        foreach($rows as $row) {
            $comp = new Component();
            $comp->createFromRow($row);
            $finalArray[] = $comp;
        }
    ...
    public function communityHasChallenge($cid) {
        $this->connect();
        $return = $this->select('community_components', array('id'), 'i', array($cid), "where cid = ? AND icon1 = 'challenge1'");
        $this->close();
    ...
    public function formObjects($type, $column, $label, $tooltip) {
        switch ($type) {
            ...
            case 'text':
                ...
                return '<section>
                            ...
                            <label class="input">
                                ...
                                <input type="text"' . $class . ' name="' . $this->id . '-' . $column . '" placeholder="Focus to view the tooltip" value="' . str_replace('"', '&quot;', $this->$column) . '">
            ...
            case 'hide':
                ...
                return $html;
                break;
            case 'include_scicrunch':
                $html = '<section>';
                $html .= '<label class="label">' . $label . '</label>';
                $html .= '<select name="' . $this->id . '-include_scicrunch">';
                if($this->include_scicrunch == 0) {
                    $html .= '<option value="0" selected="selected">Don\'t include blog posts from the main SciCrunch community</option>';
                    $html .= '<option value="1">Include blog posts from the main SciCrunch community</option>';
                } else {
                    $html .= '<option value="0">Don\'t include blog posts from the main SciCrunch community</option>';
                    $html .= '<option value="1" selected="selected">Include blog posts from the main SciCrunch community</option>';
                }
                $html .= '</select>';
                $html .= '</section>';
                return $html;
    ...
    public function bodyComponentHTML($total, $num, $actions, $components) {
        ...
        switch ($this->component) {
            ...
            case 16: //Highlight Panel 2
                ...
                $html .= $this->formObjects('color', 'color1', 'Highlight Color', 'The color of the read more button and underline');
                break;
            case 17: //Term Search Banner',
                $html .= $this->formObjects('template', '', 'Template', '');
                if ($components && count($components) > 0) {
                    $html .= $this->formObjects('position', $components, 'Place Where', '');
                }
                $html .= $this->formObjects('text', 'text1', 'Placeholder Text', 'The text shown inside the search box');
                break;
            case 21: //Goto Block',
                $html = $this->getPanelHeader($total, $num, $actions);
                $html .= '<fieldset>';
            ...
            case 37:
                ...
                $html .= $this->formObjects('text', 'text1', 'Title Text', 'The title text');
                break;
            case 38:
                $html .= $this->formObjects('template', '', 'Template', '');
                if ($components && count($components) > 0) {
                ...
                $html .= $this->formObjects('text', 'text1', 'Title Text', 'The title of the text');
                $html .= $this->formObjects('text', 'text2', 'Tag', 'The tag to search for in posts');
                $html .= $this->formObjects('text', 'text3', 'Count', 'The max number of items to display');
                $html .= $this->formObjects('color', 'color1', 'Border color', 'The color of the borders');
                $html .= $this->formObjects('text', 'color2', 'Twitter Handle', 'The handle of your twitter to link to.');
                $html .= $this->formObjects('text', 'color3', 'Twitter Widget ID', 'The widget ID for your twitter, preferred height is 380.');
                break;
            default :
                if ($this->icon1 == 'challenge1') {
                    ...
                    $html .= $this->formObjects('textarea', 'color1', 'Challenge series settings (JSON format)', 'Settings for the Challenge Series');
                    ...
                    if($this->icon1 == 'blog1' && $this->cid !== 0) $html .= $this->formObjects('include_scicrunch', 'include_scicrunch', 'Include articles from SciCrunch', '');
                    ...
                    if ($this->icon1 == 'table1' || $this->icon1 == 'challengeset1' )
                        $html .= $this->formObjects('text', 'icon3', 'Component ID for parent', 'Component ID for parent');
                ...
                if(
                    $this->icon1 != 'static' &&
                    $this->icon1 != 'files1' &&
                    $this->icon1 != "slideshow1" &&
                    $this->icon1 != "blog1" &&
                    $this->icon1 != "series1" &&
                    $this->icon1 != "event1" &&
                    $this->icon1 != 'timeline1' &&
                    $this->icon1 != 'timeline2' &&
                    $this->icon1 != 'gallery1' &&
                    $this->icon1 != 'table1' &&
                    $this->icon1 != 'doi'
                ){
    ...
    static function footerHTML($components, $community, $vars){
        ob_start();
        if (!isset($vars['stripped']) || $vars['stripped'] != 'true') {
            ...
            echo '<div style="background:#fff;height:20px"></div>';
        }

        return ob_get_clean();
    }

    public static function macroExpand($text, $data) {
        ...
        $new_text = str_replace('${portal_name}', $community->portalName, $text);

        $found = preg_match('/\$\{pmid(?:&nbsp;| )(\d+)\}/', $new_text, $matches);
        if($found == 1) {
            $matches_len = count($matches);
            for($i = 1; $i < $matches_len; $i++) {
                $pmid = $matches[$i];
                $pub_item = \helper\htmlElement("lit-item", Array("pmid" => $pmid));
                $new_text = preg_replace('/\$\{pmid(?:&nbsp;| )' . $pmid . '\}/', $pub_item, $new_text);
            }
        }
    ...
    static function getMainColor($cid) {
        $color = self::$comColors[$cid];
        if($color != null) return $color;
        $con = new Connection();
        $con->connect();
        $dbColor = $con->select('community_components', array('color1'), 'is', array($cid, 'header'), 'where cid=? and type=?');
        $con->close();
        if(is_null($dbColor)) $color = 'ffffff';
        else $color = ($dbColor[0]['color1'] ? $dbColor[0]['color1'] : 'ffffff');
        self::$comColors[$cid] = $color;
        return $color;
    }

    static function getColorStatic($component, $community, $n) {
        if(!is_null($component)) {
            return $component->getColor($n);
        }
        return self::getMainColor($community->id);
    }

    public function getColor($n) {
        switch($n) {
        case 1:
            return $this->color1 != null ? $this->color1 : self::getMainColor($this->cid);
            break;
        case 2:
            return $this->color2 != null ? $this->color2 : self::getMainColor($this->cid);
            break;
        case 3:
            return $this->color3 != null ? $this->color3 : self::getMainColor($this->cid);
            break;
        default:
            return null;
        }
