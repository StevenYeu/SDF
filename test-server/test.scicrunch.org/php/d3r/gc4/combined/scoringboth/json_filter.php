...
//    $file = '../spreadsheets/newcsvs/' . $abbr . "_". $csvdir . "/" . $file_fragment;
    ...
    // create array to store merged arrays
    $both_array = array();
    
    foreach (array($file) as $filetouse) {
        /* new code to use csv instead of json */
        $csv_array = array_map('str_getcsv', file($filetouse));
        ...
        // merge arrays
        $both_array = array_merge($csv_array, $both_array);
    }
...
//Manual Intervention,Machine Learning

    $j_array[] = array('label'=>$filename, 'Receipt'=>$filename, 'kendall'=>$line["Spearman's Rho"], 'moe'=>$line["Spearman's Rho Error"], 'n'=>$line['Number of Ligands'], 'method'=>str_replace("_based_scoring", "", $line["Method Type"]), 'manual'=>strtolower(trim($line["Manual Intervention"])), 'machine'=>strtolower(trim($line["Machine Learning"])));
} elseif ($which == 'kendall') {       
	$j_array[] = array('label'=>$line['Submission ID'], 'Receipt'=>$line['Submission ID'], 'kendall'=>$line['Kendalls Tau'], 'moe'=>$line["Kendalls Tau Error"], 'n'=>$line['Number of Ligands'], 'method'=>str_replace("_based_scoring", "", $line["Method Type"]), 'manual'=>strtolower(trim($line["Manual Intervention"])), 'machine'=>strtolower(trim($line["Machine Learning"])));
        
} else {       
    $j_array[] = array('label'=>$line['Submission ID'], 'Receipt'=>$line['Submission ID'], 'kendall'=>$line['Matthews Correlation Coefficient'], 'moe'=>0, 'n'=>$line['Number of Ligands'], 'method'=>str_replace("_based_scoring", "", $line["Method Type"]), 'manual'=>strtolower(trim($line["Manual Intervention"])), 'machine'=>strtolower(trim($line["Machine Learning"])));
