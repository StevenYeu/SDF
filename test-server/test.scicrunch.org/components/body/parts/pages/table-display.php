...
$files = $holder->getByData($component_data->id, true);
?>
<style>
...
<div class="row margin-bottom-20">
    <div class="heading heading-v1 margin-bottom-40">
        <h2><?php echo $component_data->title ?></h2>

        <p><?php echo $component_data->description ?></p>
    </div>
</div>
...
<?php
if ($component_data->color) {
    $newVars = $vars;
    $newVars['category'] = 'data';
    $newVars['nif'] = $component_data->color;
...
<hr/>
    <div class="row">
        ...
        <div class="col-md-2">
            ...
            <?php echo $search->currentSpecialFacets($vars,'/'.$community->portalName.'/about/'.$thisComp->text2.'/'.$component_data->id) ?>
            <?php 
                // Delete or otherwise suppress “Facets” from the dataset pages. #194
				if ((isset($results['facets'])) && (sizeof($results['facets']))): ?>
				<h3>Facets</h3>
				<ul class="list-group sidebar-nav-v1" id="sidebar-nav">
					...
					foreach ($results['facets'] as $column => $array) {
						...
						foreach ($array as $facet) {
							...
							echo '<li><a href="' . $search->generateSpecialURL($newVars,'/'.$community->portalName.'/about/'.$thisComp->text2.'/'.$component_data->id) . '">' . $facet['value'] . ' (' . number_format($facet['count']) . ')</a></li>';
						}
                ...
                <div class="panel-heading">
                    ...
                    <div class="pull-right">
                        <div class="btn-group navbar-right">
                            ...
                            <ul role="menu" class="dropdown-menu">
                                <li><a href="<?php echo str_replace("count=20", "count=1000", $results['export']) ?>"><i class="fa fa-cloud-download"></i>
                                        Download
                                        All</a></li>
                            </ul>
                        </div>
