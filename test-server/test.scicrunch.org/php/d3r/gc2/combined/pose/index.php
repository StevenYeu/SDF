...
//        $ligands = array("FXR_1", "AVG"); // 44 was purposely removed
    ...
    if (isset($_GET['average'])) {
        ...
        if (isset($_GET['ligand'])) {
            if (($ligand == 'Average') || ($ligand == 'Median'))
                $ligand .= " over all";
            else
                $ligand = $_GET['ligand'];
        } else {
    ...
    $json_filter = "/php/d3r/gc2/combined/pose/json_filter.php?" . implode('&', $parameters);
...
<head>
    <meta charset="UTF-8">
    <title>Pose Prediction Method - RMSD</title>
...
<h1>Grand Challenge 2</h1>
  ...
  if ($_GET['chart'] == 'closest'): ?>
<?php echo $title . $ligand . " - Closest (lowest RMSD)"; ?></h2>
...
<?php
    ...
    foreach ($ligands as $aligand) {
        ...
        if (($aligand == 'Average') || ($aligand == 'Median'))
...
?>
<input onclick="fixButton('closest');" type="submit" value="Closest" />
...
$(document).ready(function() {
   var newtable = $('#gc2results').dataTable( {
     "pageLength": 10,
     "order": [[ 5, "asc" ]]
   });
