    ...
    Circles.prototype = {
        ...
        _generateSvg: function () {
            return '<svg width="' + this._svgSize + '" height="' + this._svgSize + '">' +
                this._generatePath(100, false, this._colors[0], true) +
                this._generatePath(this._canAnimate ? 1 : this._percentage, true, this._colors[1], false) +
                '</svg>';
        },

        _generatePath: function (percentage, open, color, isOutline) {
            if (isOutline) {
                return '<path fill="transparent" stroke="' + color + '" stroke-width="' + this._strokeWidth + '" d="' + this._calculatePath(0, percentage, open) + '"/>';
            } else {
                percentage = 100 / (this._colors.length - 1);
                var HTML = '';
                var start = this._start;
                for (var i = 1; i < this._colors.length; i++) {
                    HTML += '<path fill="transparent" stroke="' + this._colors[i] + '" stroke-width="' + this._strokeWidth + '" d="' + this._calculatePath(start, percentage, open) + '"/>';
                    start = start + ((percentage / 100) * this._circ);
        ...
        _calculatePath: function (start, percentage, open) {
            var end = start + ((percentage / 100) * this._circ),
                endPrecise = this._precise(end);
            return this._arc(start, endPrecise, open);
        },

        _arc: function (start, end, open) {
            var preStart = this._precise(start);
            var endAdjusted = end - 0.001,
                longArc = end - preStart < Math.PI ? 0 : 1;

            return [
                'M',
                this._radius + this._radiusAdjusted * Math.cos(preStart),
                this._radius + this._radiusAdjusted * Math.sin(preStart),
                'A', // arcTo
